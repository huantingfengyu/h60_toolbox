...
OUT_DIR=out
KERNEL_OUT_DIR=${CUR_DIR}/${OUT_DIR}/target/product/hi3630/obj/KERNEL_OBJ
...
build_kernel() {
    ...
    echo -e "${txtylw}Building modem driver..."
    echo -e "${txtrst}"
    pushd kernel/drivers/vendor/hisi/build
    export USE_CCACHE=0
    if ! python obuild.py product=hi3630_udp acore-oam_ps -j${THREADS}; then
        echo -e "${txtred}Failed to build modem driver!"
        echo -e "${txtrst}"
        exit 1
    fi
    popd
    ...
    echo -e "${txtrst}"
    make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} O=${KERNEL_OUT_DIR} merge_hi3630_defconfig
    make ARCH=arm CROSS_COMPILE=${CROSS_COMPILE} O=${KERNEL_OUT_DIR} -j${THREADS}
    popd
...
case "$CMD" in
    clean)
        ...
        rm -rf ${CUR_DIR}/${TEMP_DIR}/*
        rm -rf ${CUR_DIR}/${OUT_DIR}/*
        popd
        echo -e "${txtgrn}Done!"
        echo -e "${txtrst}"
        ;;
    kernel)
        echo -e "${txtylw}Building kernel..."
        echo -e "${txtrst}"
        build_kernel
    ...
    *)
        ...
        echo -e "${txtblu}       ./build.sh bootimage l04    ->    creates boot.img for l04. Supported variants: l02 l04 l12"
